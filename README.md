...
The project is split into 3 distinct phases: KFG construction, C-Tree construction, and Evaluation. The bundled `mitmdump` executable is selected automatically for Windows, Linux, and macOS. On Linux and macOS, ensure the executable bit is preserved on the files in `src/main/resources/mitmproxy-5.3.0-linux` and `src/main/resources/mitmproxy-5.3.0-mac`.

# Running the Java workflow

`edu.usc.Main` remains available as the legacy entry point. For the new cross-platform entry point, configure the workflow in `config.txt` and run:
```
mvn compile exec:java
```

Set `strategy=lynx` to use the LYNX strategy (ICST 2026). Set `lynx_operation` to one of the following values:
- `all` — load the KFG, build the C-Tree, and run detection
- `ctree` — load the KFG and build the C-Tree
- `detect` — run detection using existing KFG and C-Tree files
- `draw-mbrs` — draw C-Tree MBRs in the browser
- `draw-labels` — draw KFG element labels in the browser
- `print-fns` — print processed false negatives
For example:
```properties
strategy=lynx
lynx_operation=detect
```

Set `strategy=new` to select the placeholder strategy; it currently reports that it is not implemented. To use another configuration file, run `mvn compile exec:java -Dexec.args="--config config-macos.txt"`.
...
2.	Set `strategy=lynx` and `lynx_operation=ctree`, then run `CrossPlatformMain`, or use the legacy `Main.java` workflow.
...
4.	Consecutive runs may cause mitmproxy to not close properly. If a proxy remains running, terminate the mitmproxy process with your operating system's process manager before the next experiment.
...
1.	In ``subjects.txt``, list the subject you would like to run (e.g., yumpu)
2.	Set `strategy=lynx` and `lynx_operation=detect`, then run `CrossPlatformMain`, or use the legacy `Main.java` workflow.
3.	If using Maven, run `mvn compile exec:java`.
